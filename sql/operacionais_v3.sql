...
-- =====================================================================
-- Migração única e idempotente para os módulos Almoxarifado e
-- Abastecimento. Não depende de nenhuma migração anterior ter sido
-- aplicada: cria tabelas/colunas/funções somente se ainda não existirem.
-- Pode ser executada mais de uma vez sem problema.
-- =====================================================================

-- ---------- ALMOXARIFADO: categorias e unidades ----------------------
...
-- backfill de categorias/unidades a partir dos itens já cadastrados (texto livre)
do $$ begin
  if exists (select 1 from information_schema.columns where table_schema='public' and table_name='alm_itens' and column_name='categoria') then
...
do $$ begin
  if exists (select 1 from information_schema.columns where table_schema='public' and table_name='alm_itens' and column_name='categoria') then
    update public.alm_itens i set categoria_id=c.id from public.alm_categorias c
      where i.categoria_id is null and lower(c.nome)=lower(trim(i.categoria));
  end if;
  if exists (select 1 from information_schema.columns where table_schema='public' and table_name='alm_itens' and column_name='unidade') then
    update public.alm_itens i set unidade_id=u.id from public.alm_unidades u
...
-- item sem localização passa a ser permitido (campo saiu do cadastro)
do $$ begin
  if exists (select 1 from information_schema.columns where table_schema='public' and table_name='alm_itens' and column_name='localizacao') then
...
-- ---------- ALMOXARIFADO: movimentações sem valor unitário/origem livre --
...
-- ---------- ALMOXARIFADO: inventário com contagem por item -------------

-- remove duplicidades antigas (se houver) e garante 1 linha por item/inventário
delete from public.alm_inventario_itens a using public.alm_inventario_itens b
...
alter table public.alm_inventarios add column if not exists concluido_em timestamptz;

-- ---------- ABASTECIMENTO: gás (m3/kg) e entrada nos tanques ------------
...
begin
  foreach t in array array['comb_tanques','comb_abastecimentos'] loop
    for r in
      ...
      where rel.relname=t and con.contype='c' and pg_get_constraintdef(con.oid) ilike '%unidade%'
    loop
      execute format('alter table public.%I drop constraint %I', t, r.conname);
    end loop;
    execute format('alter table public.%I add constraint %I check (unidade in (''L'',''m3'',''kg''))', t, t||'_unidade_check');
  end loop;
end $$;
...
begin
  foreach t in array array['comb_veiculos','comb_tanques','comb_abastecimentos'] loop
    for r in
      select con.conname from pg_constraint con join pg_class rel on rel.oid=con.conrelid
      where rel.relname=t and con.contype='c' and pg_get_constraintdef(con.oid) ilike '%combustivel%'
    loop
      execute format('alter table public.%I drop constraint %I', t, r.conname);
    end loop;
    execute format('alter table public.%I add constraint %I check (combustivel in (''diesel'',''gasolina'',''etanol'',''gas''))', t, t||'_combustivel_check');
  end loop;
end $$;
