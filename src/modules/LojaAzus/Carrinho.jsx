...
const FORMAS_PAGAMENTO = ["Pix", "Boleto", "Cartão", "A combinar"];
...
export function Carrinho({ itens, onAtualizarQtd, onRemover, onVoltar, onEnviar, enviando }) {
  ...
  const total = itens.reduce((s, it) => s + it.precoUnit * it.quantidade, 0);

  const confirmar = () => {
    if (!nome.trim()) { setErro("Informe seu nome."); return; }
    if (!itens.length) { setErro("O carrinho está vazio."); return; }
    setErro("");
    onEnviar({ clienteNome: nome.trim(), clienteTelefone: telefone.trim(), formaPagamento, observacoes: observacoes.trim() });
  };
